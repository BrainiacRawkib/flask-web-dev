...
def user_posts(username):
    ...
    if user is None:
        abort(404)
    posts = user_posts.posts.order_by(Post.timestamp.desc()).all()
...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('main.index', id=post.id, page=-1))
    page = request.args.get('page', 1, type=int)
    if page == -1:
        page = (post.comments.count() - 1) // current_app.config['FLASKY_PAGINATION'] + 1
...
def moderate_enable(id):
    ...
    db.session.add(comment)
    db.session.commit()
    return redirect(url_for('main.index', page=request.args.get('page', 1, type=int)))
    ...
    for query in get_debug_queries():
        if query.duration >= current_app.config['FLASKY_SLOW_DB_QUERY_TIME']:
            current_app.logger.warning(
                f'Slow query: {query.statement}: {query.parameters}: {query.duration}: {query.context}'
            )
    return response
