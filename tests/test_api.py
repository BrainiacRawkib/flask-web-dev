...
class APITestCase(unittest.TestCase):
    ...
    def test_no_auth(self):
        response = self.client.auth('/api/v1/posts/', content_type='application/json')
    ...
    def test_bad_auth(self):
        ...
        u = User(email='user@user.com', password='cat', confirmed=True, role=r)
        db.session.add(u)
        db.session.commit()

        # authenticate with bad password
        response = self.client.get('/api/v1/posts/', headers=self.get_api_headers('user@user.com', 'dog'))
        self.assertEqual(response.status_code, 401)
